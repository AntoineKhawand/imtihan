...
|---|---|---|---|
| `engineering` | App code, API routes, product features/bugs | `ROADMAP.md`, `BUGS.md` | Read/Edit/Write/Bash/Grep/Glob/WebSearch |
...
**`TEAM_CHAT.md` is the standing channel between all seven teams.** Subagents are stateless — each dispatch is a fresh instance with no memory of prior runs — so this file is what makes cross-team awareness persistent between one-shot dispatches instead of relying only on the heavier `team-sync` workflow. Every team's `.md` file instructs it to skim the last ~20 entries before starting and append a short line when it finishes, if another team would want to know — even when not explicitly asked to coordinate. It's a heads-up channel (1-3 lines, a pointer), not a report; full detail always still goes in the real domain doc.
