...
function handleGraphs(text: string): string {
  return text.replace(/\[(?:IMAGE|GRAPH|VISUAL):\s*(.*?)\]/gi, (_match, description) => {
    ...
    const prompt = (desc.length > 250 ? desc.slice(0, 250) : desc)
      + ", professional UI mockup, flat design, scientific vector illustration, high-fidelity, white background, minimalist academic aesthetic";
    const src = `https://image.pollinations.ai/prompt/${encodeURIComponent(prompt)}?width=800&height=450&nologo=true&seed=${uid}`;
