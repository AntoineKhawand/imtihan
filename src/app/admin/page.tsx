...
export default function AdminPage() {
  ...
  async function handleExtend(targetUid: string, days = 30) {
    if (!user) return;
    setExtending(targetUid);
    try {
      const token = await user.getIdToken();
      ...
      const data = await res.json();
      setUsers((prev) =>
        prev.map((u) => u.uid === targetUid ? { ...u, proExpiresAt: data.proExpiresAt } : u)
      );
    } catch (e) {
                ...
                {filtered.map((u) => (
                  <tr key={u.uid} className="hover:bg-gray-50 transition-colors">
                    <td className="px-4 py-3">
                      <p className="font-medium text-gray-900 truncate max-w-[220px]">{u.email}</p>
                      {u.displayName && <p className="text-xs text-gray-400">{u.displayName}</p>}
                    </td>
                    <td className="px-4 py-3">
                      <ProBadge expiresAt={u.proExpiresAt} />
                    </td>
                    <td className="px-4 py-3 text-gray-500">{formatDate(u.proExpiresAt)}</td>
                    <td className="px-4 py-3 text-right text-gray-500 tabular-nums">{u.examsGenerated}</td>
                    <td className="px-4 py-3 text-right">
                      <button
                        onClick={() => handleExtend(u.uid, 30)}
                        disabled={extending === u.uid}
                      ...
                      >
                        {extending === u.uid ? "Extending…" : "+ 30 days"}
                      </button>
                    </td>
                  </tr>
                ))}
