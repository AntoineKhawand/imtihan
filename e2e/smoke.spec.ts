...
test("terms page loads", async ({ page }) => {
  await page.goto("http://localhost:3005/terms");
...
test("bank page loads", async ({ page }) => {
  await page.goto("http://localhost:3005/bank");
  await expect(page.locator("body")).toBeVisible();
});

test("manifest returns 200", async ({ request }) => {
  const response = await request.get("http://localhost:3005/manifest.webmanifest");
  expect(response.status()).toBe(200);
});

test("icon-192.png exists", async ({ page }) => {
  const response = await page.request.get("http://localhost:3005/icon-192.png");
  expect(response.status()).toBe(200);
});

test("icon-512.png exists", async ({ page }) => {
  const response = await page.request.get("http://localhost:3005/icon-512.png");
  expect(response.status()).toBe(200);
});
